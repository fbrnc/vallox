...
module.exports = {
    VlxDevConstants: VlxDevConstants,
    VlxReadConstants: VlxReadConstants,
    setValue: function(WebSocket, type, value) {
        var buf = new VlxDataBuffer();
        var item = new VlxWriteItem();
        item.address = type;
        item.value = value;
        buf.appendData(item);
        WebSocket.send(buf.convertDataToBuffer(VlxDevConstants.WS_WEB_UI_COMMAND_WRITE_DATA));
    }
};
